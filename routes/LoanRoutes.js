...
router.post("/add", (req, res) => {
  const { uid, principle, interest, tenure ,emi} = req.body;

  try {
    const insertQuery = `INSERT INTO Loan (uid, principle, interest, tenure,emi) VALUES (?, ?, ?, ?,?)`;
    connection.query(
      insertQuery,
      [uid, principle, interest, tenure,emi],
      (err, results) => {
        if (err) {
